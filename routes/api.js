...
router.post('/student', function(req, res, next){
    Student.authenticate(req.body.rollno, req.body.password, function (error, user) {
        if (error || !user) {
...
router.get('/complaint', function(req, res, next){
    ...
    .exec(function(error, user){
        if(error){
            ...
            if(user === null){
                var err = new Error('Hmmm Fake login LOL');
                err.status = 400;
                return next(err);
            }else{
...
router.post('/dash', function(req, res, next){
    Complaint.find({roll : req.body.rollno}).sort({date: -1}).exec(function(error, data){
        /*if(!data){
...
router.get("/addstudent", function(req, res, next){
    ...
    function stud(rollno, password){
        this.rollno = rollno;
        this.password = password;
    }

    var data = [];
    for(var i = 17501; i <= 17590; i++){
        data.push(new stud(i ,i));
    }
    console.log(data);
